...
// ── Bank Payment Sheet ─────────────────────────────────────────────────────────
// Minimal one-row-per-employee sheet for submission to the bank for bulk
// salary transfer: name, national ID, bank, account number, net amount to pay.
async function generateBankPaymentExport(companyId, month) {
  const [paysnap, settingsSnap] = await Promise.all([
    payslipsRef(companyId, month).orderBy('lastName').get(),
    settingsRef(companyId).get(),
  ]);
  ...
  const ws = {};
  const headers = [
    'No.', 'Employee Name', 'National ID', 'Bank', 'Account Number', 'Amount to Pay (RWF)',
  ];
  ws['!cols'] = [5, 28, 20, 22, 22, 20].map(w => ({ wch: w }));

  let row = 1;

  // ── Title block ───────────────────────────────────────────────────────────
  _setCell(ws, row++, 1, `BANK PAYMENT INSTRUCTION — ${monthLabel}`, true);
  _setCell(ws, row++, 1, companyName);
  row++; // blank row
  ...
  payslips.forEach(ps => {
    const bankName = nameForCode(ps.bankCode);
    const accountNumber = ps.bankAccountNumber || '';
    const amount = _r(ps.netSalary ?? 0);
    // Employees with no bank details on file can't be wired via bank
    // transfer — skip them from the payment file rather than send an
    // unusable blank row, but keep a count so it's visible in the footer.
    if (!bankName || !accountNumber) { skipped++; return; }

    totAmount += amount;
    const cols = [
      0, // filled below with running row number after skips
      `${ps.lastName ?? ''} ${ps.firstName ?? ''}`.trim(),
      ps.nationalId ?? '',
      bankName,
      accountNumber,
  ...
  // Backfill the running "No." column now that skipped rows are excluded.
  const dataStartRow = 5;
  for (let r = dataStartRow, n = 1; r < row; r++, n++) {
    _setCell(ws, r, 1, n);
  }

  // ── Totals row ────────────────────────────────────────────────────────────
  const totals = ['', 'TOTAL', '', '', '', _r(totAmount)];
  totals.forEach((v, c) => _setCell(ws, row, c + 1, v, true));
  row += 2;

  // ── Footer ────────────────────────────────────────────────────────────────
  _setCell(ws, row++, 1, `Employees included: ${payslips.length - skipped}`);
  if (skipped > 0) {
    _setCell(ws, row++, 1, `Skipped (missing bank details): ${skipped}`);
  }
  _setCell(ws, row++, 1, `Generated by HRNovva on ${new Date().toLocaleDateString('en-GB')}`);

  ws['!ref'] = XLSX.utils.encode_range({ s: { r: 0, c: 0 }, e: { r: row, c: headers.length - 1 } });
  XLSX.utils.book_append_sheet(wb, ws, 'Bank Payment');
  wb.Props = { Title: `Bank Payment — ${month}`, Author: 'HRNovva', CreatedDate: new Date() };
...
module.exports = { generateRRAPayeExport, generatePayrollExcelExport, generateBankPaymentExport };
